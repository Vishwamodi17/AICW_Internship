...
from .forms import LoanApplicationForm, LOAN_TYPES


def fmt_inr(amount):
    """Format number in Indian number system: ₹1,50,000"""
...
# ── Landing page ───────────────────────────────────────────────
def landing(request):
    return render(request, 'loan_app/landing.html', {
        'steps':        HOW_IT_WORKS,
        'loan_types':   LOAN_TYPES,
        'mock_factors': ['Credit History', 'Total Income', 'Property Area', 'Loan Amount'],
    })
...
# ── EMI Review — intercept between form and prediction ────────
@login_required(login_url='login')
def emi_review(request):
    """
    Step 2 of the flow: user submitted the form, we show EMI
    breakdown before running the ML prediction.
    On GET  → redirect back to apply (direct access not allowed).
    On POST → validate form, compute EMI, render review page.
    """
    if request.method != 'POST':
        return redirect('apply')
    ...
    loan_type_key  = data.get('loan_type', 'home')
    loan_type_info = LOAN_TYPES.get(loan_type_key, LOAN_TYPES['home'])

    # loan_amount is now full ₹ (e.g. 1500000)
    principal   = float(data['loan_amount'])
    annual_rate = loan_type_info['rate']
    months      = float(data['loan_amount_term'])
    ...
    emi_data = {
        ...
        # Indian formatted strings
        'fmt_emi':        fmt_inr(emi),
        'fmt_total':      fmt_inr(total_payment),
        'fmt_interest':   fmt_inr(total_interest),
        'fmt_principal':  fmt_inr(principal),
        'fmt_income':     fmt_inr(total_income),
    }

    return render(request, 'loan_app/emi_review.html', {
        'form':           form,
        'form_data':      data,
        'post_data':      request.POST,   # raw POST for re-submission
...
def predict(request):
    ...
    form = LoanApplicationForm(request.POST)
    if not form.is_valid():
        # If form is invalid coming from emi_review, go back to apply
        return render(request, 'loan_app/index.html', {
            'form':       form,
            'loan_types': LOAN_TYPES,
            'prefill':    '{}',
    ...
    # EMI for result page (full rupees now)
    principal   = float(data['loan_amount'])
    rate        = loan_type_info['rate']
    months      = float(data['loan_amount_term'])
    r           = rate / 12 / 100
    if r > 0 and months > 0:
        emi_val = (principal * r * (1 + r) ** months) / ((1 + r) ** months - 1)
    else:
        emi_val = principal / months if months > 0 else 0
    ...
    context = {
        ...
        'loan_type':      loan_type_info,
        'loan_type_key':  loan_type_key,
        'emi_monthly':    fmt_inr(emi_val),
        'loan_amount_fmt':fmt_inr(principal),
    }
...
def _build_reasons(data, result, approved_prob):
    ...
    loan_amount_full= float(data['loan_amount'])          # full ₹
    loan_amount     = loan_amount_full / 1000              # in thousands for display
...
def _preprocess(data):
    ...
    applicant_inc   = float(data['applicant_income'])
    coapplicant_inc = float(data['coapplicant_income'])
    # loan_amount entered as full ₹ — model expects thousands
    loan_amount     = float(data['loan_amount']) / 1000
    loan_term       = float(data['loan_amount_term'])
